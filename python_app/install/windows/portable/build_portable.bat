...
set "APP_DIR=%CD%"
set "BUILD_VENV=%APP_DIR%\.venv-base-build"
...
"%BUILD_VENV%\Scripts\pyinstaller.exe" "install\windows\alienvox.spec" ^
    --distpath "install\windows\dist" ^
    --workpath "install\windows\build" ^
    --noconfirm
if errorlevel 1 (echo ERROR: PyInstaller build failed. & popd & exit /b 1)

echo Copying version.yaml alongside the frozen app...
copy /Y "version.yaml" "install\windows\dist\AlienVox\version.yaml" >nul

echo Zipping portable package...
if exist "install\windows\portable\AlienVox-portable-win64.zip" del "install\windows\portable\AlienVox-portable-win64.zip"
...
set "ZIP_TRIES=0"
:zip_retry
powershell -NoProfile -Command "Compress-Archive -Path 'install\windows\dist\AlienVox\*' -DestinationPath 'install\windows\portable\AlienVox-portable-win64.zip' -Force" >nul 2>nul
if exist "install\windows\portable\AlienVox-portable-win64.zip" goto zip_done
set /a ZIP_TRIES+=1
...
echo.
echo   Done: install\windows\portable\AlienVox-portable-win64.zip
