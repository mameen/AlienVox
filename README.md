...
- **SoX** ([sourceforge.net](https://sourceforge.net/projects/sox/) or `winget install ChrisBagwell.SoX`) — optional, only needed by the Qwen3-TTS ML engine (`qwen-tts`, still under evaluation — see `python_app/requirements.txt`); everything else runs fine without it, aside from a harmless warning on import.
