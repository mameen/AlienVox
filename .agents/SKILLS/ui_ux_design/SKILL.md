...
- **Playback telemetry:** play actions must emit privacy-preserving telemetry for performance tuning. Track a request id, engine/model/voice identifiers, text character/byte counts, trigger time, first-audio latency, playback end when the backend can observe it, and errors. Never record the source text itself.
