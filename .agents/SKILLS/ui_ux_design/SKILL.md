...
- **Playback telemetry:** play actions must emit privacy-preserving telemetry for performance tuning. Generate one session id at every cold app startup, then generate one play id per playback so all events from the same run and playback can be joined. Track engine/model/voice identifiers, text character/byte counts, trigger time, first-audio latency, playback end when the backend can observe it, full stack configuration, and errors. Never record the source text itself.
